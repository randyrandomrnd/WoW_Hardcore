...
--====================================================================--
-- Modified  by Yazpad; added decimalToAscii85
--
-- Usage:
-- local encoded = decimalToAscii85(<some integer>)
-- local decoded = ascii85ToBinary(encoded)
--
-- Notes: Include via toc; use globals
-- Also: This uses ascii 33(!)-117(u).  Use delimiters { } | ~ or space for delimeters. Do not use delimiters like - , @ $ % and so on.
...
function base85ToBinary(substr)
	...
	local bin_short = decimalToBinary(num)
	local pre = ""
	for i=1,(32-#bin_short)/8 do
	  pre = pre .. "00000000"
	end
	return pre .. bin_short
end
...
function decimalToAscii85(dec)
      local b85 = decimalToBase85(dec)
      local final = ""
      for i=1,#b85 do 
	      local char = tostring(b85[i]+33)
	      final = final .. char:char()
...
function ascii85ToBinary(str)
	...
	for i=1,l,c do 
		local sub = str:sub(i,i+c-1)
		final = final .. base85ToBinary(sub)
	end 
	return final 
end 
